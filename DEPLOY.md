...
| `ANTHROPIC_API_KEY` | question generation (not needed when running through Bedrock) |
| `AWS_BEDROCK_REGION` | optional; set it (e.g. `eu-west-1`) to route every Claude call through Amazon Bedrock instead |
| `AWS_BEARER_TOKEN_BEDROCK` | the Bedrock API key — required when `AWS_BEDROCK_REGION` is set |
| `ANTHROPIC_MODEL` | optional; on Bedrock ids carry a routing prefix (e.g. `global.anthropic.claude-sonnet-4-6`) |
