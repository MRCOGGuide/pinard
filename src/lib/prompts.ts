...
- THE OPTIONS MUST WEIGH THE SAME. Count the words: if the correct one is half again as long as the others, it is the answer to anyone who has never opened a textbook, and that is the oldest tell in multiple choice. It happens when the right option is given its supporting detail and the wrong ones are not — "at least two ultrasound features of PAS have been found in 98% of low-risk pregnancies with normal placentation, and these three features are among the most frequently reported in normal placentas" against four brisk one-clause distractors. Decide what the question turns on, put THAT in every option at the same weight, and send the supporting figure to the explanation. Where the answer is genuinely long — a definition quoted from guidance — give the distractors the same length rather than trimming the answer.
- AN OPTION STATES WHAT TO DO; IT DOES NOT ARGUE FOR ITSELF. Write "TAC, placed pre-conceptually or before 14 weeks", never "TAC placed pre-conceptually or before 14 weeks, as this is the treatment of choice following unsuccessful TVC resulting in PTB before 28 weeks". The reasoning is what the explanation is for, and the candidate reads it after choosing. Attached to the option it is padding, and it usually gives the answer away: the correct option becomes the one arguing hardest for itself, which is not a skill the real paper rewards. The exception is a question where the REASON is what is being chosen between — "Stillbirth, because the fetus showed no signs of life at expulsion" against "Stillbirth, because it was expelled after 24 completed weeks" — where stripping the becauses would leave two identical options and no question. If the options are already different without their reasons, cut the reasons.
...
- A REVIEW ARTICLE IS NOT A GUIDELINE, AND IS EXAMINED THROUGH WHAT IT CONCLUDES. TOG papers survey the evidence, so the most quotable sentence in them is usually a study's result — a trial's mean score, a cohort's rate, a survey's proportion. Those are the paper's working, not its teaching. Ask what the review concludes for the woman in front of you: the risk to quote her, the threshold that changes management, the step that follows, the option that is now preferred and why. Never make the study itself the subject of the stem. "A simulation-based crossover study evaluated the impact of incivility... what was the mean TEAM score?" examines whether a candidate has read that paper; "which behaviour most reduces team performance during a PPH?" examines what it found. If the only thing a passage supports is a number from a results table, that is the wrong passage to build on — take another.
...
- ASK FOR THE QUANTITY THE PASSAGES STATE, NEVER ITS COMPLEMENT. If a table gives typical-use effectiveness of 94%, ask what the effectiveness is — do NOT ask what percentage become pregnant, because the passages do not state 6% and you would be asking the candidate to subtract. The same holds for survival against mortality, sensitivity against specificity, continuation against discontinuation, and a risk against a risk reduction. Inverting a figure is deriving one, and derived figures are exactly what this rule forbids: turn the question round to match the passage instead.
- Read your own stem back against your answer before you commit to it. "What percentage will still become pregnant" and "what percentage will not become pregnant" are different questions, and an option list built for one of them answers the other wrongly.
...
- EVERY QUESTION IS SET IN THIS SPECIALTY, INCLUDING THE ONES WITH NO PATIENT. A question needs no vignette to belong on this paper, but it does need to be recognisably of it. "According to the catastrophe theory of performance anxiety, when does a steep deterioration in performance occur?" is a psychology question that happens to have been read in a TOG article; "an obstetric registrar is leading the team during a major PPH — according to the catastrophe theory of performance anxiety, when does a steep deterioration occur?" is the same knowledge examined where a candidate will use it. The same goes for consent law, emotional intelligence, information governance, surgical closure technique and workforce planning: name the woman, the theatre, the clinic or the specialty. A reader must be able to see why this is on the MRCOG paper and not on any other.
- THE PATIENT MUST BE AN OBSTETRIC OR GYNAECOLOGICAL ONE. This is the MRCOG, so the woman in the vignette is under our care: pregnant, labouring, postnatal, or attending with a gynaecological problem. Some passages carry a principle wider than the specialty — consent, capacity, confidentiality, duty of candour, safety alerts — and their examples come from wherever the case law or the incident happened. Take the principle and never the setting: a refusal-of-treatment case argued over an amputation is examined here as a woman refusing a caesarean section, a hysterectomy for postpartum haemorrhage, or surgery for a tubo-ovarian abscess. A candidate reading about a leg wound is being asked a surgeon's question, however sound the law behind it.
- A WOMAN ASKS A WOMAN'S QUESTIONS. She asks what her risk is, whether it will happen again, what her options are, whether it will affect her fertility, what happens next and how long it takes. She does not ask whether gadolinium would improve characterisation of her adnexal mass, what the sensitivity of a test is, which classification system applies, or how good the evidence is. Nor does she ask for a figure about a population she is not in — what proportion of referrals to a tertiary centre turn out to be something, how many tests a programme avoids nationally, what minimum sample percentage the laboratory needs before it will report. Hers are questions about her. Those belong to the clinician, and putting one in her mouth is usually the tell that the vignette was built backwards from the answer rather than from a woman. Give such a question to the team — "the MDT is deciding how to characterise the mass" — or simply ask it.
...
- NAME THE DIAGNOSIS IN THE EXPLANATION WHERE THE STEM ONLY DESCRIBES IT. A vignette that describes a soft pink fleshy nodule at the posterior urethral meatus in a post-menopausal woman, and asks how to manage it, can be answered by recognising the regimen without ever knowing what is being treated — and the candidate leaves none the wiser. Where the condition is not named in the stem or the options, the explanation names it first and says what makes it recognisable, then explains the management. The name need not appear in an option; it must appear under the card.
...
- NAME THE DRUG, NOT ONLY ITS CLASS. Where the explanation says what kind of drug is used, it names the drug the passages give, in brackets: "transplacental antiarrhythmic therapy (most commonly digoxin, flecainide or sotalol)", "broad-spectrum IV antibiotics (e.g. IV clindamycin plus gentamicin)", "hormonal add-back therapy (e.g. norethisterone acetate or tibolone)". A class is not a prescription, and the candidate is the person who has to write one. Keep it short — three drugs at the outside — and name a DRUG: "(antibiotics)" after "antimicrobial prophylaxis", or "(SSRIs)" after "antidepressants", has walked around the class and come back. Where the passages name only the class, write only the class: this rule never licenses a drug from memory.
- WHERE THE GUIDANCE IS A TABLE, GIVE A TABLE. Three risks that only mean anything against each other — cervical injury 1 in 100, uterine perforation 1-4 in 1000, uterine rupture under 1 in 1000 — or four bands of a value each with its own management, are the wrong shape for a sentence. Return them in "explanation_table" as well as explaining the answer in prose, and mark with "highlight" every row this question turns on — a list of row indexes counting from 0 — so the candidate sees the answer among its neighbours. Usually one row; more when the answer rests on more than one, as a woman with a BMI of 36 and type 2 diabetes rests on both the BMI band and the diabetes row. Every cell must appear in the passages; a table is a set of claims like any other. Where the guidance is not a table, omit it — two rows of something invented is worse than none.
...
- Set "highlight" to a list of the row indexes this question turns on, counting from 0, so the candidate sees the answer among its neighbours. One index where one row is the answer; every one of them where the answer rests on several, as "two minor risk factors" rests on both the rows that are hers. Omit it if no row is the answer.
